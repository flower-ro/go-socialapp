...
func MainForTest(priv string, pub string, user string, index int, idEncode string) {
	logLevel = "DEBUG"
	...
	if err := fs.Connect(); err != nil {
		...
		fs.Close(0)
		return
	} else if err = doHandshake(fs, *keys.NewKeyPair(), priv, pub, user, index, idEncode); err != nil {
		fs.Close(0)
